...
class NurseSchedulingFactory2:
    ...
    def get_optimization_model(self):
        ...
        obj_func = self.shift_on_requests()
        obj_func += self.shift_off_requests()
        constraints, penalty = self.cover(formulation="slack")
        model += constraints
        obj_func += penalty
    ...
    def min_consecutive(self):
        ...
        for _, nurse in self.data.staff.iterrows():
            ...
            for i, shift in enumerate(nurse_shifts):
                ...
                any_off_before = cp.any(nurse_shifts[:i] == FREE)
                start_of_working_period = (shift != FREE) & (nurse_shifts[i-1] == FREE)
                nurse_constraint.append((any_off_before & start_of_working_period).implies(cp.all(nurse_shifts[i:i+min_days] != FREE)))
    ...
    def min_consecutive_off(self):
        ...
        for _, nurse in self.data.staff.iterrows():
            ...
            for i, shift in enumerate(nurse_shifts):
                ...
                any_shift_before_is_assigned = cp.any(nurse_shifts[:i] != FREE)
                is_start_of_free_period = (shift == FREE) & (nurse_shifts[i - 1] != FREE)

                nurse_constraint.append((any_shift_before_is_assigned & is_start_of_free_period).implies(
                    cp.all(nurse_shifts[i:i + min_days] == FREE)
                ))
    ...
    def shift_on_requests(self, decision=False):
        """
            If the specified shift is not assigned to the specified employee on the specified day
                then the solution's penalty is the specified weight value.
        ...
        for _, request in self.data.shift_on.iterrows():
            ...
            if decision:
                ...
                exprs.append(constraint)
            else: # penalty
                exprs.append(request['Weight'] * (self.nurse_view[n, day] != shift))
    ...
    def shift_off_requests(self, decision=False):
        """
            If the specified shift is assigned to the specified employee on the specified day
                then the solution's penalty is the weight value.
        """
    ...
    def cover(self, formulation="penalty"):
        """
        If the required number of staff on the specified day for the specified shift is not assigned
            then it is a soft constraint violation
